...
class ProductController extends IndexController {
	...
	public function addsave() {
		...
		$darr = array();
		$darr['ptype'] = kpost('ptype', '');
		$darr['title'] = kpost('title', '');
		$darr['img'] = kpost('img', '');
		$darr['intro'] = kpost('intro', '');
		$darr['punit'] = kpost('punit', '');
		$darr['oamt'] = kpost('oamt', '');
		$darr['amt'] = kpost('amt', '');
		$darr['content'] = kpost('content', '');
		$darr['ordering'] = kpost('ordering', '');
	...
	public function showall() {
		...
		foreach ($arr as $key => $v) {
			...
			// 	$arr[$key]['cateid'] = '-';
			// }
			$arr[$key]['istop'] = $arr[$key]['istop'] > 0 ? layspan('置顶推荐', 'danger') : '-';
			// $arr[$key]['addtime'] = date('Y-m-d H:i', $arr[$key]['addtime']);
			if ($v['states'] == 1) {
				$arr[$key]['states'] = layspan('上架中', 'danger');
			}
			// if ($v['states'] == 2) {
			// 	$arr[$key]['states'] = '禁用';
			// }
			if ($v['states'] == 0) {
				$arr[$key]['states'] = layspan('下架', 'aaa');
			}
